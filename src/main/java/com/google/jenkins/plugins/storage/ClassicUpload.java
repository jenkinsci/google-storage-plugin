...
/**
 * This upload extension implements the classical upload pattern where a user
 * provides an Ant-style glob, e.g. ** / *.java relative to the build workspace,
 * and those files are uploaded to the storage bucket.
 */
public class ClassicUpload extends AbstractUpload {
    ...
    /**
     * Construct the classic upload implementation from the base properties and
     * the glob for matching files.
     */
    ...
    public ClassicUpload(String bucket,
        ...
        this.sourceGlobWithVars = firstNonNull(pattern, sourceGlobWithVars);
